...
> Sigak은 중요한 기술 변화를 이해하기 위해 retrieval-augmented, graph-aware, evaluated LLM enrichment를 결합한 연구형 기술 뉴스 인사이트 플랫폼이다.
...
| 기준 | 질문 |
| --- | --- |
| 사실성 | source article과 모순되지 않는가? |
| 유용성 | 기술 독자가 실제로 맥락을 이해하는 데 도움이 되는가? |
| 구체성 | 일반론이 아니라 기사와 관련된 구체적 설명인가? |
| 명확성 | 초심자도 읽을 수 있지만 얕지 않은가? |
| 관계 품질 | 관련 기사/개념 연결이 타당한가? |
...
### 단계 R1: End-to-End Collection과 Enrichment
...
### 단계 R4: RAG와 Graph-Aware Generation
